...
float jacobi(
    int n, Matrix* M1, Matrix* M2, Matrix* M3, Matrix* M4, Matrix* M5, Matrix* M6, Matrix* M7);
...
float jacobi(
    int nn, Matrix* a, Matrix* b, Matrix* c, Matrix* p, Matrix* bnd, Matrix* wrk1, Matrix* wrk2) {
  int i, j, k, n, imax, jmax, kmax;
  double gosa = 0.0f, s0, ss;
...
#pragma omp parallel shared(imax, jmax, kmax, a, b, c, p, wrk1, wrk2, bnd, gosa) private(s0, ss)
    {
#pragma omp for reduction(+ : gosa) schedule(guided)
      for(i = 1; i < imax; i++)
        for(j = 1; j < jmax; j++)
          for(k = 1; k < kmax; k++) {
            s0 = MR(a, 0, i, j, k) * MR(p, 0, i + 1, j, k) +
                 ...
                 MR(b, 0, i, j, k) * (MR(p, 0, i + 1, j + 1, k) - MR(p, 0, i + 1, j - 1, k) -
                                      MR(p, 0, i - 1, j + 1, k) + MR(p, 0, i - 1, j - 1, k)) +
                 MR(b, 1, i, j, k) * (MR(p, 0, i, j + 1, k + 1) - MR(p, 0, i, j - 1, k + 1) -
                                      MR(p, 0, i, j + 1, k - 1) + MR(p, 0, i, j - 1, k - 1)) +
                 MR(b, 2, i, j, k) * (MR(p, 0, i + 1, j, k + 1) - MR(p, 0, i - 1, j, k + 1) -
...
#pragma omp for schedule(guided)
      for(i = 1; i < imax; i++)
        for(j = 1; j < jmax; j++)
          for(k = 1; k < kmax; k++) MR(p, 0, i, j, k) = MR(wrk2, 0, i, j, k);
    }
  } /* end n loop */
